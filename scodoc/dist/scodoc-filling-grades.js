...
`,fe={name:"scodoc-remplisseur-automatique-notes",version:"1.5.0-scodoc",description:"",main:"src/index.js",scripts:{build:"vite build",start:"vite",dev:"vite",release:"standard-version"},keywords:[],author:"",license:"ISC",devDependencies:{"csvjson-csv2json":"^5.0.6",encoding:"^0.1.13","standard-version":"^9.5.0",vite:"^4.4.11"}};let ee=!1;const pe=()=>ee,X=i=>ee=i;(async function(){if(A.listGradesRows.length===0&&!A.formContainer)return;document.getElementsByTagName("body")[0].insertAdjacentHTML("beforeend",de),A.dragAndDropArea=document.querySelector("[data-drag-n-drop-area]"),A.resetContainer=document.querySelector("[data-restart-upload-container]"),A.firstStep=document.querySelector("[data-first-step]"),A.uploadBtn=document.querySelector("[data-upload-btn]"),A.resetContainer.style.display="none",document.querySelector("[data-project-name]").textContent+=` v${fe.version.replace("-scodoc","")}`,await Promise.resolve().then(()=>me),W(document,"change","#grades_file",p=>{document.querySelectorAll("[data-etudid]").forEach(h=>{h.style.backgroundColor=""});const y=document.querySelector('input[name="empty_val"]:checked').value||"ABS";ue({target:p,valForMissingGrade:y,dom:A})}),W(document,"click","[data-restart]",()=>{X(!1),Z()}),W(document,"click","[data-force-save]",()=>{ce()})})();const K=document.querySelector("[data-drag-n-drop-area]");["dragend","dragleave"].forEach(i=>{K.addEventListener(i,p=>{p.preventDefault(),p.currentTarget.querySelector(".tp-upload-btn").classList.remove("over"),p.currentTarget.classList.remove("over")})}),K.addEventListener("dragover",i=>{i.preventDefault(),i.currentTarget.querySelector(".tp-upload-btn").classList.add("over"),i.currentTarget.classList.add("over")}),K.addEventListener("drop",i=>{i.preventDefault(),i.currentTarget.querySelector(".tp-upload-btn").classList.remove("over"),i.currentTarget.classList.remove("over"),i.dataTransfer.items&&[...i.dataTransfer.items].forEach((p,y)=>{if(p.kind==="file"){X(!0);const h=i.currentTarget.querySelector("input[type='file']");h.setAttribute("files",i.dataTransfer.files),h.files=i.dataTransfer.files,h.dispatchEvent(new Event("change",{bubbles:!0}))}})});const me=Object.freeze(Object.defineProperty({__proto__:null},Symbol.toStringTag,{value:"Module"}));U.hasUsedDnDrop=pe,U.setHasUsedDnDrop=X,Object.defineProperty(U,Symbol.toStringTag,{value:"Module"})});
