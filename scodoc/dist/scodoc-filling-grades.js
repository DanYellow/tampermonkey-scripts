...
Soit votre évaluation n'a pas la bonne note maximale sur ScoDoc soit vous n'entrez pas les notes de la bonne évaluation sur ScoDoc.
                `),J();return}V=[],se(j,g);const c=document.querySelector("[data-template-id='unknown-student']"),a=document.querySelector("[data-list-unknown-students]");a.replaceChildren();const o=document.querySelector("[data-nb-unknown-students]");o.textContent=V.length,V.forEach(d=>{const e=c.content.cloneNode(!0);e.querySelector("li").textContent=d,a.append(e)}),Array.from(document.querySelectorAll(".note[value='']")).forEach(d=>{d.focus(),d.value=m,d.blur()}),w.firstStep.style.display="none",w.resetContainer.style.display="block",Z()&&(w.resetContainer.querySelector("[data-force-save]").style.display="inline-block")}},E.readAsArrayBuffer(v)},K=(i,m,g,v)=>{i.addEventListener(m,function(N){let A=N.target;for(;A&&A!==this;)A.matches(g)&&v.call(A,N),A=A.parentNode})},ue=()=>{document.querySelectorAll("[data-etudid]").forEach(i=>{i.focus(),i.blur()})},ce=`<style>\r
